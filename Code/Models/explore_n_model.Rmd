...
opts_knit$set(root.dir = "/Users/mdturse/Desktop/Analytics/Sparkfund/")
...
```
  ...
  Read in the .csv files and save it as one .rds file.
```{r message=FALSE}
...
# View(hd2 %>% filter(model == "13H2B97AS"))
...
rec_steps <-
  ...
  step_nzv(all_predictors()) %>%  # helps reduce the number of variables with many NA values
  # step_sqrt(all_numeric()) %>%  # puts less emphasis on "outliers"
...
```
  ...
  Add `weight_col`, as this is a potential parameter to be used. The vlaue of 100 is arbitrarily chosen to make predicting `failure` more important than `non_failure`.
```{r}
...
automl_models_h2o <-
  h2o.automl(x = x,
             ...
             max_runtime_secs = 3600, #3600, # 1 hour
             # max_runtime_secs = 60, # 1 hour
             max_models = 10,
             # stopping_metric = "deviance", # ? AUTO ?
             stopping_metric = "AUTO", # ? AUTO ?
             seed = 123456789
             )
...
# twoClassSummary(data = error_tbl_h2o %>% select(obs, pred, p0, p1),
#                 lev = levels(error_tbl_h2o$obs)
#                 )

# postResample(pred = error_tbl_h2o$pred,
#              obs = error_tbl_h2o$obs
#              )
